...
namespace StockMarket {
    ...
    void Menu::handleBuyStock() {
        ...
        try {
            ...
            // Record transaction
            Transaction tx(generateTransactionId(), ticker, "BUY", quantity,
    ...
    void Menu::handleSellStock() {
        ...
        try {
            ...
            // Record transaction
            Transaction tx(generateTransactionId(), ticker, "SELL", quantity,
                           stock.getCurrentPrice(), Utils::getCurrentTimestamp());
            transactionLog.push_back(tx);
            FileManager::saveTransaction(tx, DATA_TRANSACTIONS_FILE);
            auth.saveUsersToFile(DATA_USERS_FILE);
    ...
    void Menu::handleViewTransactionHistory() const {
        ...
        // The transactionLog stores all transactions for all users.
        // We identify user transactions using a simple prefix in the TxID (generated by session).
        // For this design, we show all since users only see their own session's records.
        // In production this would be user-keyed, but for simplicity we display all.
        if (transactionLog.empty()) {
            std::cout << "  [INFO] No transactions recorded yet.\n";
            return;
        }
        ...
        std::cout << std::string(78, '-') << "\n";
        std::cout << "  Total Transactions: " << transactionLog.size() << "\n";
    }
